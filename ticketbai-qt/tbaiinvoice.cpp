...
QByteArray TbaiInvoice::getId() const
{
  ...
  for (int i = 0 ; i < 13 && i < signature.length() ; ++i)
    out += signature[i];
  out += separator;
  return QByteArray::fromStdString(out.toStdString());
}

QByteArray TbaiInvoice::getIdWithCRC() const
{
  QByteArray base = getId();

  return base + generateCRC(base);
}
...
QByteArray TbaiInvoice::generateCRC(const QByteArray &input) const
{
  static const int crc8Table[] = {
  ...
  for (int i = 0 ; i < input.length() ; ++i)
    crc = crc8Table[(crc ^ input[i]) & 0xff];
  result = QByteArray::number(crc);
