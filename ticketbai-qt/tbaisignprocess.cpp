...
bool TbaiSignProcess::findSignatureIn(const QDomDocument& signatureDocument)
{
  ...
  signatureCandidates = signatureDocument.elementsByTagName("ds:SignatureValue");
  if (signatureCandidates.count() > 0)
  {
    QString signature = signatureCandidates.at(0).toElement().text();

    emit generatedSignature(QByteArray(signature.toStdString().c_str()));
    return true;
  }
  else
    emit failed("could not find <ds:SignatureValue> in the xml generated by autofirma.jar");
  return false;
}
...
void TbaiSignProcess::onAutofirmaDone(int code, QProcess::ExitStatus)
{
  ...
  if (code == 0)
  {
    ...
    outputFile.close();
    if (findSignatureIn(signatureDocument))
